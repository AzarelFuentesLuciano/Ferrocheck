...
$test('El camino app_shell ya no está bloqueado', !str_contains($source, "if (\$legacy['contenidoModulo'] === '')"));
$test('Usa la vista FerroCheck sin shell', str_contains($source, "require __DIR__ . '/../Views/inventario/partials/ferrocheck-content.php';"));
$test('Captura contenido antes del bridge', strpos($source, '$contenidoModulo = $this->renderFerroCheckContent($ferroSeccion);') < strpos($source, 'new LegacyRenderBridge()'));
$test('Asigna el contenido capturado al contexto legacy', str_contains($source, "'contenidoModulo' => \$contenidoModulo"));
