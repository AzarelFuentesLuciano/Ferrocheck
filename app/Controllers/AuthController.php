...
use App\Auth\Csrf;
use App\Services\{AuthService,GeneralAuditService};
...
final class AuthController
{
    ...
    public function dispatch(string$method,array$query,array$post,array$server):void
    {
        ...
        $current=$this->auth->current();if($current!==null){header('Location: '.BASE_URL.'/index.php?modulo=dashboard',true,302);return;}
    ...
    {
        if(!$this->csrf->validate((string)($post['_csrf']??''))){$this->flash('La sesión del formulario expiró.');$this->redirectLogin();return;}
        try{$url=$this->auth->login((string)($post['usuario']??''),(string)($post['password']??''),isset($post['return'])?(string)$post['return']:null,$server);$id=(int)($this->session['user_id']??0);$this->audit->record($id,'auth.login','usuario',$id,'exito',[],[],[],$this->ip($server));$this->csrf->rotate();header('Location: '.$url,true,303);}catch(\Throwable$e){$this->audit->record(null,'auth.login','usuario',null,'rechazado',[],[],['usuario_hash'=>hash('sha256',mb_strtolower(trim((string)($post['usuario']??''))))],$this->ip($server));$this->flash($e instanceof \DomainException?$e->getMessage():'No fue posible iniciar sesión.');$this->redirectLogin();}
    }
    private function logout(string$method,array$post,array$server):void
    {
        ...
        $id=(int)($this->session['user_id']??0);$this->auth->logout();$this->audit->record($id?:null,'auth.logout','usuario',$id?:null,'exito',[],[],[],$this->ip($server));$this->csrf->rotate();header('Location: '.BASE_URL.'/index.php?modulo=auth',true,303);
    }
    private function redirectLogin():void{header('Location: '.BASE_URL.'/index.php?modulo=auth',true,303);}
